; ModuleID = 'ECTLang module'
source_filename = "ECTLang module"

@0 = private unnamed_addr constant [4 x i8] c"%f\0A\00", align 1
@1 = private unnamed_addr constant [4 x i8] c"%d\0A\00", align 1
@2 = private unnamed_addr constant [4 x i8] c"%d\0A\00", align 1
@3 = private unnamed_addr constant [4 x i8] c"%f\0A\00", align 1
@4 = private unnamed_addr constant [4 x i8] c"%f\0A\00", align 1
@5 = private unnamed_addr constant [4 x i8] c"%f\0A\00", align 1

define void @main() {
body:
  %0 = call i32 (ptr, ...) @printf(ptr @0, double 0x4032CCCCC4000000)
  %1 = call i32 (ptr, ...) @printf(ptr @1, i32 -1)
  %2 = call i32 (ptr, ...) @printf(ptr @2, i32 4)
  %3 = call i32 (ptr, ...) @printf(ptr @3, double 0x4058F99990000000)
  %4 = call i32 (ptr, ...) @printf(ptr @4, double 1.000000e+00)
  %5 = call i32 (ptr, ...) @printf(ptr @5, double 0x3FECCCCCE0000000)
  ret void
}

declare i32 @printf(ptr, ...)
